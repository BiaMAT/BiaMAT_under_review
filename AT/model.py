...
class Model(object):
  ...
  def __init__(self, main_num_classes, aux_num_classes, n=4):
    """ResNet constructor.
    """
  ...
  def _build_model(self):
    ...
    with tf.variable_scope('input'):
      ...
      y_input_ori, self.y_input_aux = tf.split(self.y_input, 2, axis=0)
      self.is_training = tf.placeholder(tf.bool, shape=None)
      self.alpha = tf.placeholder(tf.float32, shape=None)
      self.y_input_ori = tf.cond(self.is_training, lambda: y_input_ori, lambda: self.y_input)
    ...
    with tf.variable_scope('logit'):
      x_ori, x_aux = tf.split(x, 2, axis=0)
      x_ori = tf.cond(self.is_training, lambda: x_ori, lambda: x)
      self.pre_softmax = self._fully_connected(x_ori, self.main_num_classes)
      self.pre_softmax_aux = self._fully_connected(x_aux, self.aux_num_classes)
    self.predictions = tf.argmax(self.pre_softmax, 1)
    self.correct_prediction = tf.equal(self.predictions, self.y_input_ori)
    ...
    with tf.variable_scope('costs'):
      self.y_xent = tf.nn.sparse_softmax_cross_entropy_with_logits(
          logits=self.pre_softmax, labels=self.y_input_ori)
      ...
      self.xent_aux = tf.reduce_sum(self.y_xent_aux, name='y_xent_aux')
      self.mean_xent_aux = tf.reduce_mean(self.y_xent_aux)

      self.xent_merge = self.xent + self.alpha * self.xent_aux
      self.mean_xent_merge = self.mean_xent + self.alpha * self.mean_xent_aux
  ...
  def _residual(self, x, in_filter, out_filter, stride,
    ...
    with tf.variable_scope('sub_add'):
      if in_filter != out_filter:
        ...
        #orig_x = tf.nn.avg_pool(orig_x, stride, stride, 'VALID', data_format='NCHW')
        orig_x = tf.pad(
