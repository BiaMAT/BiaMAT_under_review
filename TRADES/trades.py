...
def trades_loss_BiaMAT(model,
    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                ...
                loss_kl += F.cross_entropy(logits_aux, y[split[0]:])
    ...
    loss_natural_default = F.cross_entropy(logits_nat, y[:split[0]])
    loss_robust_default = (1.0 / batch_size) * criterion_kl(F.log_softmax(logits, dim=1),
                                                  F.softmax(logits_nat, dim=1))
    loss_robust_aux = F.cross_entropy(logits_aux, y[split[0]:])
    loss_default = loss_natural_default + beta * loss_robust_default
    loss = loss_default + alpha * loss_robust_aux

    return loss, alpha * loss_robust_aux
